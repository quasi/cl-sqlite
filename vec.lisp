...
(telos:defun/i create-vector-table (db name columns &key (if-not-exists nil))
...
Example:
  ...
  (:feature inquisitio-vec)
  (:purpose "Generate CREATE VIRTUAL TABLE DDL for sqlite-vec vec0 tables")
...
(telos:defun/i vector-search (db table query-vector &key (k 10) (column 'embedding) (output-columns '(rowid distance)))
...
Returns a list of lists (rows)."
  (:feature inquisitio-vec)
  (:purpose "Execute k-nearest-neighbor search against a vec0 virtual table")
...
(telos:defun/i vec-add (db vec1 vec2)
  "Returns the element-wise addition of two vectors."
  (:feature inquisitio-vec)
  (:purpose "Wrap sqlite-vec vec_add scalar function")
  (blob-to-float-vector (wrap-scalar-vec-func db "vec_add" vec1 vec2)))

(telos:defun/i vec-sub (db vec1 vec2)
  "Returns the element-wise subtraction of two vectors."
  (:feature inquisitio-vec)
  (:purpose "Wrap sqlite-vec vec_sub scalar function")
  (blob-to-float-vector (wrap-scalar-vec-func db "vec_sub" vec1 vec2)))

(telos:defun/i vec-distance-L2 (db vec1 vec2)
  "Calculates L2 distance between two vectors."
  (:feature inquisitio-vec)
  (:purpose "Wrap sqlite-vec vec_distance_L2 scalar function")
  (wrap-scalar-vec-func db "vec_distance_L2" vec1 vec2))

(telos:defun/i vec-distance-cosine (db vec1 vec2)
  "Calculates Cosine distance between two vectors."
  (:feature inquisitio-vec)
  (:purpose "Wrap sqlite-vec vec_distance_cosine scalar function")
  (wrap-scalar-vec-func db "vec_distance_cosine" vec1 vec2))

(telos:defun/i vec-distance-hamming (db vec1 vec2)
  "Calculates Hamming distance between two vectors."
  (:feature inquisitio-vec)
  (:purpose "Wrap sqlite-vec vec_distance_hamming scalar function")
  (wrap-scalar-vec-func db "vec_distance_hamming" vec1 vec2))

(telos:defun/i vec-normalize (db vec)
  "Returns the normalized vector."
  (:feature inquisitio-vec)
  (:purpose "Wrap sqlite-vec vec_normalize scalar function")
  (blob-to-float-vector (wrap-scalar-vec-func db "vec_normalize" vec)))

(telos:defun/i vec-length (db vec)
  "Returns the length (dimension) of the vector."
  (:feature inquisitio-vec)
  (:purpose "Wrap sqlite-vec vec_length scalar function")
  (wrap-scalar-vec-func db "vec_length" vec))

(telos:defun/i vec-to-json (db vec)
  "Returns the JSON string representation of the vector."
  (:feature inquisitio-vec)
  (:purpose "Wrap sqlite-vec vec_to_json scalar function")
  (wrap-scalar-vec-func db "vec_to_json" vec))

(telos:defun/i vec-slice (db vec start end)
  "Returns a slice of the vector."
  (:feature inquisitio-vec)
  (:purpose "Wrap sqlite-vec vec_slice scalar function")
